package com.ahmed.m.hassaan.face_mask_detection
...
class MainActivity : AppCompatActivity() {
        ...
        btnChangeCamera.setOnClickListener {
            if (cameraView.facing == Facing.BACK) {
                try {
                    ...
                    cameraView.facing = Facing.BACK
                    Toast.makeText(applicationContext, "Front Camera Not Found", Toast.LENGTH_SHORT)
                        .show()
                }
            } else {
                try {
                    ...
                    cameraView.facing = Facing.FRONT
                    Toast.makeText(applicationContext, "Back Camera Not Found", Toast.LENGTH_SHORT)
                        .show()
                }
            }
    ...
    private fun processBitmap(bitmap: Bitmap, faceDetector: FaceDetector): MutableList<Box> {
        ...
        for (i in 0 until faces.size()) {
            ...
            prediction = if (with > without) {
                "With Mask : " + String.format("%.1f", with * 100) + "%"
            } else {
                "Without Mask : " + String.format("%.1f", without * 100) + "%"
            }
            boundingBoxList.add(Box(RectF(left, top, right, bottom), prediction, with > without))
        }
